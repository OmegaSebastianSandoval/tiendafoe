 ...
 */
class Administracion_reporteanalistaController extends Administracion_mainController
{
	...
	public function init()
	{
		...
		$this->route = "/administracion/reporteanalista";
	...
	public function indexAction()
	{
		...
		$this->_view->lists = $this->mainModel->getListPagesAnalistas($filters, $order, $start, $amount);
		...
		$hoy = date("YmdHis");
		if ($excel == 1) {
			$this->_view->lists = $this->mainModel->getListAnalistas($filters, $order);
	...
	protected function getFilter()
	{
		...
		if (Session::getInstance()->get($this->namefilter) != "") {
			...
			if ($filters->fecha1ra != '' && $filters->fecha2ra != '') {
				$filtros = $filtros . " AND items.fecha >= '$filters->fecha1ra 00:00:00' AND items.fecha <= '$filters->fecha2ra 23:59:59'";
			}
	...
	protected function filters()
	{
		if ($this->getRequest()->isPost() == true) {
			...
			$parramsfilter['fecha1ra'] = $this->_getSanitizedParam("fecha1ra");
			$parramsfilter['fecha2ra'] = $this->_getSanitizedParam("fecha2ra");
