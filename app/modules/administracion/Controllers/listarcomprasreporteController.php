 ...
 */
class Administracion_listarcomprasreporteController extends Administracion_mainController
{
	...
	public function init()
	{
		...
		$this->route = "/administracion/listarcomprasreporte";
	...
	public function indexAction()
	{
		...
		$this->_view->lists = $this->mainModel->getListPagesCompras($filters, $order, $start, $amount);
		...
		$hoy = date("YmdHis");
		if ($excel == 1) {
			$this->_view->lists = $this->mainModel->getListCompras($filters, $order);
	...
	protected function getFilter()
	{
		...
		if (Session::getInstance()->get($this->namefilter) != "") {
			...
			if ($filters->fecha1lc != '' && $filters->fecha2lc != '') {
				$filtros = $filtros . " AND items.fecha >= '$filters->fecha1lc 00:00:00' AND items.fecha <= '$filters->fecha2lc 23:59:59'";
			}
	...
	protected function filters()
	{
		if ($this->getRequest()->isPost() == true) {
			...
			$parramsfilter['fecha1lc'] = $this->_getSanitizedParam("fecha1lc");
			$parramsfilter['fecha2lc'] = $this->_getSanitizedParam("fecha2lc");
